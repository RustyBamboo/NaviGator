...
class PingerMission:
    ...
    def get_observation_poses(self):
        ...
        distance_test = np.array([np.linalg.norm(pose - (self.gate_poses[0] +
                                                         self.OBSERVE_DISTANCE_METERS * self.g_perp)),
                                  np.linalg.norm(pose - (self.gate_poses[0] -
                                                         self.OBSERVE_DISTANCE_METERS * self.g_perp))])
        ...
        if self.USE_CLOSE_POINTS:
            ...
            if self.negate:
                self.observation_points = (np.append((self.gate_poses[0] -
                                                      self.OBSERVE_DISTANCE_METERS * self.g_perp), 0),
                                           np.append((self.gate_poses[2] -
                                                      self.OBSERVE_DISTANCE_METERS * self.g_perp), 0))
            else:
                self.observation_points = (np.append((self.gate_poses[0] +
                                                      self.OBSERVE_DISTANCE_METERS * self.g_perp), 0),
    ...
    def get_colored_buoys(self):
        ...
        if totems.found:
            ...
            if np.linalg.norm(mil_tools.rosmsg_to_numpy(sorted_circle[0].position)[:2] -
                              estimated_circle_buoy) < self.MAX_CIRCLE_BUOY_ERROR:
    ...
    def circle_buoy(self):
        if self.circle_totem is not None:
            # Circle around totem
            yield self.navigator.move.look_at(self.circle_totem).set_position(self.circle_totem)\
