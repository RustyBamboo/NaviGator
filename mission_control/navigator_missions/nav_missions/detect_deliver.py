...
class DetectDeliverMission:
        ...
        if shapes.found:
            for shape in shapes.shapes.list:
                ...
                if normal_res.success:
                    enu_cam_tf = yield self.navigator.\
                        tf_listener.get_transform('/enu', '/' +
                                                  shape.header.frame_id, shape.header.stamp)
    ...
    def circle_search(self):
        ...
        yield self.navigator.move.look_at(platform_np).set_position(platform_np).\
            backward(self.circle_radius).yaw_left(90, unit='deg').go(move_type="drive")
